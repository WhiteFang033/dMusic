...
import RemoveFromPlaylist from '@/components/songOptions/RemoveFromPlaylist'
const PlaylistItem = ({index, title, artist, songSource, setSongArray, playlistName, fetchPlaylistsSongs}) => {
  ...
  return (
    ...
    <div onMouseOver={()=>{setCursorStatus(true)}} onMouseOut={()=>{setCursorStatus(false)}} className="flex w-[90%] min-h-[70px] bg-zinc-900 rounded-xl px-5 my-2 items-center border-2 border-zinc-900 justify-between hover:bg-purple-700 hover:border-purple-700 transition-all delay-75">
        ...
        <div>
        <RemoveFromPlaylist playlistName = {playlistName} songTitle = {title} songArtist = {artist} fetchPlaylistsSongs={fetchPlaylistsSongs}/>
        </div>
      </div>
  )
}
